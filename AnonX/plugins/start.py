        ...
        if name[0:3] == "sta":
            ...
            def get_stats():
                ...
                for vidid, count in list_arranged.items():
                    ...
                    if vidid == "telegram":
                        msg += f"🔗[قناة السورس](https://t.me/UI_XB) ** ᴩʟᴀʏᴇᴅ {count} ᴛɪᴍᴇs**\n\n"
                    else:
...
💖 بحث بواسطة {config.MUSIC_BOT_NAME}"""
            key = InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton(
                            text="• قناة السورس •", url="https://t.me/UI_XB"
                        ),
                    ],
                ]
            )
        ...
        if config.START_IMG_URL:
            try:
                ...
                user = message.from_user.id
                if int(user) == dev_owner: 
                   return await message.reply(f"**𖢿 | : مرحبا حبيبي زد إي مطور السورس{message.from_user.mention}\n𖢿 | : كل اقسام التحكم بالبوتات\n𖢿 | : تستطيع التحكم بكل البوتات عن طريق هذه الازرار**",reply_markup=OwnerM)
